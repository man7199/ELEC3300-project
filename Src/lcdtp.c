...
void LCD_DrawTriangleup ( uint16_t usC1, uint16_t usP1, uint16_t usC2, uint16_t usP2, uint16_t usColor ){
		int i=(usC1+usC2)/2;
		int x=0;
			for (int y=usP1;y<usP2;y++,x++){
				LCD_DrawLine(i-x,y,i+x,y,usColor);			
			}
		
		}
void LCD_DrawTriangledown ( uint16_t usC1, uint16_t usP1, uint16_t usC2, uint16_t usP2, uint16_t usColor ){
		int i=(usC1+usC2)/2;
		int x=0;
			for (int y=usP2;y>usP1;y--,x++){
				LCD_DrawLine(i-x,y,i+x,y,usColor);			
			}
...
void LCD_DrawBigChar ( uint16_t usC, uint16_t usP, const char cChar )
{
	uint8_t ucTemp, ucRelativePositon, ucPage, ucColumn;
	...
	LCD_OpenWindow ( usC, usP, 50, 50 );
	
	LCD_Write_Cmd ( CMD_SetPixel );	
	
	for ( ucPage = 0; ucPage < 50; ucPage ++ )
	{
		ucTemp = ucAscii_1608 [ ucRelativePositon ] [ ucPage ];
		
		for ( ucColumn = 0; ucColumn < 50; ucColumn ++ )
		{
			if ( ucTemp & 0x01 )
				LCD_Write_Data ( 0x001F );
...
void LCD_DrawBigString ( uint16_t usC, uint16_t usP, const char * pStr )
{
	while ( * pStr != '\0' )
	{
		if ( ( usC - LCD_DispWindow_Start_COLUMN + 50 ) > LCD_DispWindow_COLUMN )
		{
			usC = LCD_DispWindow_Start_COLUMN;
			usP += 50;
		}
		
		if ( ( usP - LCD_DispWindow_Start_PAGE + 50 ) > LCD_DispWindow_PAGE )
		{
			usC = LCD_DispWindow_Start_COLUMN;
			usP = LCD_DispWindow_Start_PAGE;
		}
		
		LCD_DrawBigChar ( usC, usP, * pStr );
...
void LCD_DrawBigDot(uint16_t x, uint16_t y, uint16_t usColor)	
{	
	LCD_DrawDot ( x-1, y-1, usColor );

  LCD_DrawDot (   x, y-1, usColor );

 LCD_DrawDot ( x+1, y-1, usColor );

  LCD_DrawDot ( x-1,   y, usColor );  
